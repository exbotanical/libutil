...
static void test_array_init(void) {
  ...
  eq_num(array_size(array), 0, "initializes the array's length to zero");
...
static void test_array_size(void) {
  ...
  eq_num(array_size(array), 0, "retrieves initial size");
  array_push(array, "test");
  eq_num(array_size(array), 1, "tracks size pointer");
...
static void test_array_get(void) {
  array_t *array = array_init();
  eq_null(array_get(array, 0), NULL, "returns NULL if empty");
  eq_null(array_get(array, 4), NULL, "index larger than len returns NULL");
  eq_null(array_get(array, -1), NULL, "-1 index returns NULL");
  eq_null(array_get(array, -4), NULL,
          "negative index larger than len returns NULL");
  ...
  eq_null(array_get(array, 4), NULL, "index larger than len returns NULL");
  eq_num((int)array_get(array, -1), 12, "-1 index returns last element");
  eq_num((int)array_get(array, 1), 11, "index returns correct element");
  eq_num((int)array_get(array, -2), 11, "index returns correct element");
...
static void test_array_includes(void) {
  ...
  eq_true(array_includes(array, int_comparator, (void *)'z'),
          "returns true when it includes the element");
  eq_false(array_includes(array, int_comparator, (void *)'n'),
...
static void test_array_find(void) {
  array_t *array = make_test_array();
  eq_num((int)array_find(array, int_comparator, (void *)'z'), 2,
         "returns index when it finds the element");
  eq_num(array_find(array, int_comparator, (void *)'n'), -1,
         "returns -1 when it does not find the element");

  array_free(array, NULL);

  array_t *strarr = array_collect("92Q", "dew");
  eq_num((int)array_find(strarr, str_comparator, (void *)"dew"), 1,
...
static void test_array_push(void) {
  ...
  for (size_t i = 0; i < 3; i++) {
    ...
    eq_true(array_push((array_t *)array, el), "returns true when successful");
    eq_num(array->size, i + 1, "increases the array's length by one");
    eq_num((int)array->state[i], (int)el,
...
static void test_array_pop(void) {
  ...
  for (; i > 0; i--) {
    eq_num((int)array_pop(array), values[i - 1],
           "removes the element from the end of the array");
    eq_num((int)((__array_t *)array)->size, i - 1,
...
static void test_array_pop_empty(void) {
  ...
  eq_null(array_pop(array), NULL, "returns NULL from an empty array");
...
static void test_array_shift(void) {
  ...
  eq_num((int)array_shift(array), 'x', "shift assert");
  eq_num(internal->size, 2, "shift assert");
  eq_num((int)internal->state[0], 'y', "shift assert");
  eq_num((int)internal->state[1], 'z', "shift assert");

  eq_num((int)array_shift(array), 'y', "shift assert");
  eq_num((int)internal->size, 1, "shift assert");
  eq_num((int)internal->state[0], 'z', "shift assert");

  eq_num((int)array_shift(array), 'z', "shift assert");
  eq_num(internal->size, 0, "shift assert");
...
static void test_array_shift_empty(void) {
  ...
  eq_null(array_shift(array), NULL, "shift on an empty array returns NULL");
...
static void test_array_slice(void) {
  ...
  eq_num(sliced->size, 3, "slices the range of elements inclusively");

  for (size_t i = 0; i < 3; i++) {
    eq_num((int)sliced->state[i], (int)((__array_t *)array)->state[i + 1],
...
static void test_array_slice_negative(void) {
  ...
  eq_num(sliced->size, 5, "slices the range of elements inclusively");
...
static void test_array_remove(void) {
  ...
  eq_true(array_remove(array, 2), "returns true when the element was removed");

  char values[] = {'x', 'y', '1', '2', '3'};
  for (size_t i = 0; i < internal->size; i++) {
    eq_num((int)internal->state[i], values[i],
...
static void test_array_remove_not_found(void) {
  ...
  eq_false(array_remove(array, 12),
...
static void test_array_map(void) {
  ...
  eq_num(transformed->size, internal->size,
         "has the same length as the input array");

  for (size_t i = 0; i < 3; i++) {
    eq_num((int)transformed->state[i], (int)internal->state[i] + MAPPER_INC,
...
static void test_array_filter(void) {
  ...
  eq_num(transformed->size, 3, "has a length of filtered elements only");
  for (size_t i = 0; i < transformed->size; i++) {
    void *el = array_get(array, i);

    if ((int)el > 'A') {
      eq_num((int)transformed->state[i], (int)el,
...
static void test_foreach_macro(void) {
  ...
  foreach (array, x) {
    eq_num(x + 1, array_get(array, x), "iterates the array");
  }

  size_t count = 0;
  foreach_i(array, x, 1) { count += 1; }

  eq_num(2, count, "starts at the provided count");
...
static void test_has_elements_macro(void) {
  ...
  eq_num(has_elements(array), 0, "has_elements returns false when arr is NULL");
  eq_num((!has_elements(array)), 1,
         "negated has_elements returns true when arr is NULL");

  array = array_init();
  eq_num(has_elements(array), 0,
...
static void test_array_collect(void) {
  ...
  eq_num(array_size(collected), 3, "contains all and only the elements added");

  eq_str("a", array_get(collected, 0), "has the expected element");
  eq_str("b", array_get(collected, 1), "has the expected element");
  eq_str("c", array_get(collected, 2), "has the expected element");
...
static void test_array_concat(void) {
  ...
  eq_num(array_size(concatenated), 6, "has the expected size");
  foreach (concatenated, i) {
    eq_str(array_get(concatenated, i), v[i], "contains %s at index %d", v[i],
           i);
  }
...
static void test_array_get_negative_idx(void) {
  ...
  eq_str(array_get(arr, -1), "x",
         "retrieves the last (and only) element given index -1");

  array_push(arr, "y");

  eq_str(array_get(arr, -1), "y",
         "retrieves the new last element given index -1");
  eq_str(array_get(arr, -2), "x",
